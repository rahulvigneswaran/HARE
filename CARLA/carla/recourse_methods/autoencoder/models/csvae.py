...
class CSVAE(nn.Module):
    ...
    def push_to_gpu(self, device):
        self.encoder_xy_to_w.to(device) 
        self.mu_xy_to_w.to(device)
        self.logvar_xy_to_w.to(device)
        self.encoder_x_to_z.to(device)
        self.mu_x_to_z.to(device)
        self.logvar_x_to_z.to(device)
        self.encoder_y_to_w.to(device)
        self.mu_y_to_w.to(device)
        self.logvar_y_to_w.to(device)
        self.decoder_zw_to_x.to(device)
        self.mu_zw_to_x.to(device)
        self.logvar_zw_to_x.to(device)
        self.decoder_z_to_y.to(device)
        self.device = device
        ...
        for i in trange(epochs):
            for x, y in train_loader:
                (
                    ...
                    y_recon_loss_val,
                ) = csvae_loss(self, x.to(self.device), y.to(self.device))
