...
def hyper_sphere_coordindates(n_search_samples, instance, high, low, seed, p_norm=2):
...
def growing_spheres_search(
    ...
    # divide up keys
    keys_mutable_continuous = sorted(list(set(keys_mutable) - set(binary_cols)), key=keys_correct.index)
    keys_mutable_binary = sorted(list(set(keys_mutable) - set(continuous_cols)), key=keys_correct.index)
    ...
    while (not counterfactuals_found) and (count < max_iter):
        ...
        candidate_counterfactuals_continuous, _ = hyper_sphere_coordindates(
            n_search_samples, instance_mutable_replicated_continuous, high, low, seed, p_norm
        )
        ...
        #TODO: original GS perturbed categorical also, currently setting them as unperturbed
