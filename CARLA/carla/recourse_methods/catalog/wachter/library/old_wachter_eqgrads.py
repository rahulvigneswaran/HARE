...
def wachter_recourse(
    ...
    while f_x_new <= DECISION_THRESHOLD:
        ...
        while f_x_new <= 0.5 and it < n_iter:
            ...
            # loss.backward()
            # #zero out gradient of immutable features
            # x_new.grad[:,immutables] = 0.0

            #equalizing gradients
            loss1 = loss_fn(f_x_loss, y_target)
            loss1.backward()
            x_new.grad[:, immutables] = 0.0
            #get norm of gradient
            old_grad = x_new.grad.clone()
            old_grad_norm = torch.linalg.norm(old_grad, ord=2, dim=1).item()
            x_new_enc = reconstruct_encoding_constraints(
                x_new, cat_feature_indices, binary_cat_features
            )
            cost = cost_fn(x_new_enc, x)
            cost.backward()
            x_new.grad[:, immutables] = 0.0
            new_grad = (x_new.grad - old_grad).clone()
            new_grad_norm = torch.linalg.norm(new_grad, ord=2, dim=1).item()
            val = lamb*old_grad_norm/new_grad_norm if new_grad_norm>1e-08 else 0
            x_new.grad = old_grad + (val)* new_grad
