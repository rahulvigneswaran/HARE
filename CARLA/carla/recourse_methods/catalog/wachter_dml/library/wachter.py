...
def wachter_recourse(
    ...
    if not next(torch_model.parameters()).is_cuda:
        torch_model = torch_model.to(device)
    if (dml_model is not None) and (not next(dml_model.parameters()).is_cuda):
        print("sending DML model to GPU")
    ...
    while f_x_new <= DECISION_THRESHOLD:
        ...
        while f_x_new <= 0.5 and it < n_iter:
            ...
            if dml_model is not None:
                phi_x = dml_model(x).squeeze(0)
                phi_x_new_enc = dml_model(x_new_enc).squeeze(0)
                cost = cost_fn(phi_x_new_enc, phi_x)
            else:    
