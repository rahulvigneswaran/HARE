...
from carla.recourse_methods.catalog.wachter_dml.library import wachter_recourse
...
class WachterDML(RecourseMethod):
    """
    ...
    _DEFAULT_HYPERPARAMS = {
        ...
        "immutables": "_optional_",
        "lr": 0.01,
        "lambda_": 0.01,
        "n_iter": 2500,
        "t_max_min": 0.7,
        "cost_fn": lambda x,y: torch.linalg.norm(x - y, ord=1),
        "clamp": True,
        "loss_type": "BCE",
        "y_target": [0, 1],
    ...
    def __init__(self, mlmodel, dml_model, hyperparams: Optional[Dict] = None):
        ...
        self._immutables = checked_hyperparams["immutables"]
        ...
        self._cost_fn = checked_hyperparams["cost_fn"]
    ...
    def get_counterfactuals(self, factuals: pd.DataFrame) -> pd.DataFrame:
        ...
        immutables = self._mlmodel.data.immutables if self._immutables is None else self._immutables
        encoded_immutables = [encoded_feature_names[self._mlmodel.data.categorical.index(name)] if name in self._mlmodel.data.categorical else name for name in immutables]
        immutables_indices = [factuals.columns.get_loc(name) for name in encoded_immutables]
