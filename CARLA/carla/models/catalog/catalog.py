        ...
        **kws,
        ...
        if load_online:
            self._model = load_online_model(
                model_type, data.name, ext, cache, self._models_home, **kws
            )
    ...
    def train(
        ...
        if not force_train:
            self._model = load_trained_model(
                save_name=save_name, data_name=self.data.name, backend=self.backend, models_home=self._models_home
            )
